...
use crate::utils::constants::{
    ACCEPTED_COMPOSE_FILE_NAMES, COMBINED_TERMINAL_COLS, COMBINED_TERMINAL_ROWS,
    CREATED_FILE, CREATED_STACK, EXITED, PROGRESS_TERMINAL_ROWS, RUNNING, UNKNOWN,
};
...
impl Stack {
    ...
    pub async fn validate(&mut self) -> Result<()> {
        ...
        if !self.name.chars().all(|c| c.is_ascii_lowercase() || c.is_ascii_digit() || c == '_' || c == '-') {
...
impl Stack {
    ...
    pub async fn save(&mut self, is_add: bool) -> Result<()> {
        ...
        let yaml = self.compose_yaml().await?;
        fs::write(&compose_path, yaml).await
        ...
        for line in stdout.lines() {
            ...
            match serde_json::from_str::<ComposePsItem>(line) {
                Ok(item) => {
                    ...
                    let ports: Vec<String> = item.ports
                        .split(", ")
                        .filter(|s| s.contains("->"))
                        .map(|s| s.to_string())
                        .collect();
                ...
                Ok(compose_list) => {
                    for item in compose_list {
                        ...
                        if let Some(stack) = stack_list.get_mut(&item.name) {
                            ...
                            // Add unmanaged stack
                            let mut stack = Stack::new(ctx.clone(), item.name.clone(), endpoint.clone());
                            stack.status = status;
                            stack.config_file_path = Some(item.config_files.clone());
