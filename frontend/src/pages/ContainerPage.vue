        ...
        <div>
            ...
            <div class="shadow-box big-padding status-box mb-3">
                <div class="mb-2">
                    ...
                    <span v-if="health" class="badge me-1" :class="healthStyle">{{ health }}</span>
                    <a
                        v-for="port in ports"
                        :key="port"
...
import { parseDockerPort, getContainerLogsTerminalName } from "../../common/util-common";
...
export default {
        ...
        terminalRoute() {
            if (this.endpoint) {
                return {
                    ...
                    params: { stackName: this.stackName, serviceName: this.serviceName, type: "bash", endpoint: this.endpoint },
                };
            }
            return {
                name: "containerTerminal",
                params: { stackName: this.stackName, serviceName: this.serviceName, type: "bash" },
            };
        },
    },
    mounted() {
    ...
    methods: {
        ...
        restart() {
            ...
            this.$root.emitAgent(this.endpoint, "restartService", this.stackName, this.serviceName, (res) => {
        ...
        start() {
            this.processing = true;
            this.$root.emitAgent(this.endpoint, "startService", this.stackName, this.serviceName, (res) => {
                this.processing = false;
                if (res.ok) {
        ...
        stop() {
            this.processing = true;
            this.$root.emitAgent(this.endpoint, "stopService", this.stackName, this.serviceName, (res) => {
                this.processing = false;
                if (res.ok) {
        ...
        forcePull() {
            this.processing = true;
            this.$root.emitAgent(this.endpoint, "pullService", this.stackName, this.serviceName, (res) => {
                this.processing = false;
                if (res.ok) {
